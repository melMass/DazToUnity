...
namespace Daz3D
{
    public static class DTUMaterialExtensions
    {
        public static string GetMaterialDir(this DTUMaterial material, DTUFile dtuFile)
        {
        ...
        /// <returns></returns>
        public static bool IsHair(this DTUMaterial dtuMaterial)
        {
            ...
            if (
                ...
                || valueLower.Contains("moustache") || assetNameLower.EndsWith("moustache") ||
                matNameLower.Contains("moustache")
        ...
        public static bool IsSkin(this DTUMaterial dtuMaterial)
        {
            ...
            if (dtuMaterial.MaterialType == "omUberSurface" || dtuMaterial.MaterialType == "omHumanSurface" ||
                dtuMaterial.MaterialType == "PBRSkin")
            {
                if (dtuMaterial.IsWet())
                {
                    return false;
                }

                if (dtuMaterial.IsHair())
                {
                    return false;
                }

                if (dtuMaterial.IsSclera())
                {
                    return false;
                }
                ...
                if (matNameLower.Contains("iris") || matNameLower.Contains("eyelash") ||
                    matNameLower.Contains("teeth") || matNameLower.Contains("nail") ||
                    matNameLower.Contains("tongue") || matNameLower.Contains("mouth") || matNameLower.Contains("pupil"))
                {
                    return false;
                }
            ...
            return dualLobeSpecularWeight.TextureExists() || dualLobeSpecularReflectivity.TextureExists();
        }
        ...
        /// <param name="dtuMaterial"></param>
        /// <returns></returns>
        public static bool IsWet(this DTUMaterial dtuMaterial)
        {
            var matNameLower = dtuMaterial.MaterialName.ToLower();
            var assetNameLower = dtuMaterial.AssetName.ToLower();
            var valueLower = dtuMaterial.Value.ToLower();
            ...
            if (matNameLower.Contains("cornea") || matNameLower.Contains("eyemoisture") ||
                matNameLower.Contains("eyereflection"))
            {
                return true;
            }
        ...
        public static bool IsSclera(this DTUMaterial dtuMaterial)
        {
            var matNameLower = dtuMaterial.MaterialName.ToLower();
            var assetNameLower = dtuMaterial.AssetName.ToLower();
            var valueLower = dtuMaterial.Value.ToLower();
            ...
            return matNameLower.Contains("sclera");
        }
        ...
        /// <returns></returns>
        public static Material ConvertToUnityIrayUber(this DTUMaterial dtuMaterial, string materialDir)
        {
            ...
            Utilities.Log("Converting to Unity Iray Uber");
            var matNameLower = dtuMaterial.MaterialName.ToLower();
            var assetNameLower = dtuMaterial.AssetName.ToLower();
            var valueLower = dtuMaterial.Value.ToLower();
            ...
            var baseMixingVal = (baseMixingProp.Exists
                ? (int) baseMixingProp.Value.AsDouble
                : (int) DTUBaseMixing.PBRMetalRoughness);
            ...
            isHair = dtuMaterial.IsHair();
            isSkin = dtuMaterial.IsSkin();
            isWet = dtuMaterial.IsWet();
            isSclera = dtuMaterial.IsSclera();

            //Swap shaders if we need to

            if (isHair)
            {
                shaderName = Constants.shaderNameHair;
            }
            else if (isSkin)
            {
                //if we're skin, force a specular workflow as well
                isSpecular = true;
                shaderName = Constants.shaderNameIraySkin;
                isDoubleSided = false;
                isTransparent = false;
                isTranslucent = true;
            }
            ...
            {
                shaderName = Constants.shaderNameWet;
            }
            else
            {
                ...
                if (isTranslucent)
                {
                    ...
                    {
                        Debug.LogWarning(
                            "Using translucency with metal is not supported, swapping to specular instead for mat: " +
                            dtuMaterial.MaterialName);
                    }

                    //if we're translucent, force into a specular workflow
                    isSpecular = true;
                    shaderName = Constants.shaderNameSpecular;
                }
                else if (isSpecular)
                {
                    shaderName = Constants.shaderNameSpecular;
                ...
                {
                    shaderName = Constants.shaderNameMetal;
                }
                else
                {
            ...
            if (shader == null)
            {
                Debug.LogError("Failed to locate shader: " + shaderName + " for mat: " +
                               dtuMaterial.MaterialName);
                return null;
            }
            ...
            if (isHair)
            {
                ...
                mat.SetTexture("_CutoutOpacityMap",
                    ImportTextureFromPath(cutoutOpacity.Texture, materialDir, record, false, true));
                mat.SetTexture("_GlossyRoughnessMap",
                    ImportTextureFromPath(glossyRoughness.Texture, materialDir, record, false, true));
            ...
            else
            {
                ...
                if (dualLobeSpecularWeight.Float > 0f)
                {
                    ...
                    {
                        Debug.LogWarning(
                            "Dual Lobe support is only available on the skin shader currently for mat: " +
                            dtuMaterial.MaterialName);
                    }
                    ...
                    if (mat.HasProperty("_DualLobeSpecularWeight"))
                    {
                        ...
                        mat.SetTexture("_DualLobeSpecularReflectivityMap",
                            ImportTextureFromPath(dualLobeSpecularReflectivity.Texture, materialDir, record, false,
                                true));
                        ...
                        if (baseMixing == DTUBaseMixing.PBRSpecularGlossiness)
                        {
                            ...
                            specularLobe1RoughnessTexture = ImportTextureFromPath(specularLobe1Glossiness.Texture,
                                materialDir, record, false, true);
                            specularLobe1RoughnessTexture = ImportTextureFromPath(specularLobe2Glossiness.Texture,
                                materialDir, record, false, true);
                        }
                        else
                        {
                            ...
                            specularLobe1RoughnessTexture = ImportTextureFromPath(specularLobe1Roughness.Texture,
                                materialDir, record, false, true);
                            specularLobe1RoughnessTexture = ImportTextureFromPath(specularLobe2Roughness.Texture,
                                materialDir, record, false, true);
                        }
                    ...
                    {
                        Debug.LogWarning("Shader: " + shaderName +
                                         " doesn't support dual lobe support yet for mat: " +
                                         dtuMaterial.MaterialName);
                    }
                }
                ...
                switch (baseMixing)
                {
                    ...
                    case DTUBaseMixing.Weighted:
                        ...
                        if (glossyWeight.TextureExists())
                        {
                            //this is an inverted map where 1 is smooth and 0 is rough
                            glossinessMap =
                                ImportTextureFromPath(glossyWeight.Texture, materialDir, record, false, true);
                ...
                if (isSpecular)
                {
                    ...
                    mat.SetTexture("_SpecularColorMap",
                        ImportTextureFromPath(glossySpecular.Texture, materialDir, record));
            ...
            ToggleCommonMaterialProperties(ref mat, matNameLower, isTransparent, isDoubleSided,
                hasDualLobeSpecularWeight, hasDualLobeSpecularReflectivity, sortingPriority, hasGlossyLayeredWeight,
                hasGlossyColor);
            ...
            if (record.Tokens.Count > 0)
                DTUImporter.EventQueue.Enqueue(record);

            return mat;
        }

        public static Material ConvertToUnityPBRSkin(this DTUMaterial dtuMaterial, string materialDir)
        {
            Utilities.Log("Converting to Unity PBRSkin");
            var matNameLower = dtuMaterial.MaterialName.ToLower();
            var assetNameLower = dtuMaterial.AssetName.ToLower();
            var valueLower = dtuMaterial.Value.ToLower();
        ...
        /// <param name="dTUMaterial"></param>
        /// <returns></returns>
        public static Material ConvertToUnityDazStudioDefault(this DTUMaterial dtuMaterial, string materialDir)
        {
            ...
            bool isTransparent = opacityStrength.TextureExists() || opacityStrength.Float < 1.0f ||
                                 (cutoutOpacity.Exists && cutoutOpacity.TextureExists());
            ...
            else
            {
                ...
                mat.SetFloat("_HeightOffset", 0.25f);
                mat.SetTexture("_AlphaMap",
                    ImportTextureFromPath(opacityStrength.Texture, materialDir, record, false, true));
                ...
                if (shaderLightingModel != DTULightingModel.Matte)
                {
                    ...
                    mat.SetTexture("_SpecularColorMap",
                        ImportTextureFromPath(specularColor.Texture, materialDir, record));

                    mat.SetFloat("_SpecularStrength", specularStrength.Float);
                    mat.SetTexture("_SpecularStrengthMap",
                        ImportTextureFromPath(specularStrength.Texture, materialDir, record, false, true));
                }
                ...
                if (shaderLightingModel == DTULightingModel.Skin)
                {
                    ...
                    mat.SetTexture("_ThicknessMap",
                        ImportTextureFromPath(thickness.Texture, materialDir, record, false, true));
                }
                ...
                if (mat.HasProperty("_Emission"))
                {
                    ...
                    if (mat.HasProperty("_EmissionStrength"))
                    {
                        ...
                        {
                            mat.SetTexture("_EmissionStrengthMap",
                                ImportTextureFromPath(ambientStrength.Texture, materialDir, record, false, true));
                        }
                    }
                    else
                    {
        ...
        /// <param name="dTUMaterial"></param>
        /// <returns></returns>
        public static Material ConvertToUnityPBRSP(this DTUMaterial dtuMaterial, string materialDir)
        {
            var metallicWeight = dtuMaterial.Get("Metallic Weight");
            var diffuseColor = dtuMaterial.Get("Diffuse Color");
            ...
            var shaderName = Constants.shaderNameMetal;
        ...
        /// <param name="dTUMaterial"></param>
        /// <returns></returns>
        public static Material ConvertToUnityOmUberSurface(this DTUMaterial dtuMaterial, string materialDir)
        {
            var diffuseColor = dtuMaterial.Get("Diffuse Color");
            var opacityStrength = dtuMaterial.Get("Opacity Strength");
            ...
            bool isHair = dtuMaterial.IsHair();
            bool isSkin = dtuMaterial.IsSkin();
            bool isWet = dtuMaterial.IsWet();
            bool isSclera = dtuMaterial.IsSclera();
            ...
            var shaderName = Constants.shaderNameSpecular;
            ...
            if (isHair)
            {
                ...
                mat.SetTexture("_GlossyRoughnessMap",
                    ImportTextureFromPath(glossiness.Texture, materialDir, record, false, true));
            ...
            else
            {
                ...
                mat.SetTexture("_AmbientOcclusionMap",
                    ImportTextureFromPath(occlusion.Texture, materialDir, record, false, true));
            }
        ...
        public static Material ConvertToUnityBlendedDualLobeHair(this DTUMaterial dtuMaterial, string materialDir)
        {
            ...
            string shaderName = Constants.shaderNameHair;
            var shader = Shader.Find(shaderName);
            if (shader == null)
            {
                Debug.LogError("Failed to locate shader: " + shaderName + " for mat: " +
            ...
            mat.SetTexture("_CutoutOpacityMap",
                ImportTextureFromPath(cutoutOpacity.Texture, materialDir, record, false, true));
            mat.SetTexture("_GlossyRoughnessMap",
                ImportTextureFromPath(baseRoughness.Texture, materialDir, record, false, true));
        ...
        public static Material ConvertToUnityOOTHairblendingHair(this DTUMaterial dtuMaterial, string materialDir)
        {
        ...
        public static Material ConvertToUnity(this DTUMaterial dtuMaterial, DTUFile dtuFile, bool regenerateMaterials)
        {
            ...
            else if (dtuMaterial.MaterialType == "OOT Hairblending Hair" ||
                     (dtuMaterial.MaterialType == "Cap" && dtuMaterial.Get("Cap Base Texture").Exists))
            {
            ...
            else
            {
                ...
                Debug.LogWarning("Unknown material type: " + dtuMaterial.MaterialType + " for mat: " +
                                 dtuMaterial.MaterialName + " using default");
                materialType = DTUMaterialType.DazStudioDefault;
                //return null;
            }
            ...
            if (materialType == DTUMaterialType.IrayUber)
            {
                ...
                var uberMat = dtuMaterial.ConvertToUnityIrayUber(materialDir);
                if (uberMat != null)
                {
            ...
            {
                var defaultMat = dtuMaterial.ConvertToUnityDazStudioDefault(materialDir);
                if (defaultMat != null)
                {
            ...
            {
                var pbrspMat = dtuMaterial.ConvertToUnityPBRSP(materialDir);
                if (pbrspMat != null)
                {
            ...
            {
                var localMat = dtuMaterial.ConvertToUnityOmUberSurface(materialDir);
                if (localMat != null)
                {
            ...
            {
                var localMat = dtuMaterial.ConvertToUnityOOTHairblendingHair(materialDir);
                if (localMat != null)
                {
                    SaveMaterialAsAsset(localMat, materialPath);
                    return localMat;
            ...
            {
                var localMat = dtuMaterial.ConvertToUnityBlendedDualLobeHair(materialDir);
                if (localMat != null)
                {
                    SaveMaterialAsAsset(localMat, materialPath);
                    return localMat;
            ...
            {
                var localMat = dtuMaterial.ConvertToUnityPBRSkin(materialDir);
                if (localMat != null)
                {
                    SaveMaterialAsAsset(localMat, materialPath);
                    return localMat;
            ...
            Debug.LogError("Unsupported materialType: " + materialType + " raw shade ris: " +
                           dtuMaterial.MaterialType);
            return null;
        }
            ...
            bool hasGlossyColor = false)
        {
            if (isTransparent)
            {
                ...
                mat.renderQueue = (int) RenderQueue.Transparent + sortingPriority;
            }
            else
            {
                ...
                mat.renderQueue = (int) RenderQueue.Geometry + sortingPriority;
            }
            ...
            if (isDoubleSided)
            {
                ...
                {
                    Debug.LogWarning(
                        "Material is both double sided and transparent, you will have rendering artifacts against the two sides where they are alphad, for mat: " +
                        mat.name);
                    mat.SetFloat("_ZWrite", 1f);
                    mat.SetFloat("_TransparentZWrite", 1f);
                }
        ...
        public static void SaveMaterialAsAsset(Material mat, string materialPath)
        {
            if (mat == null)
            {
                return;
            ...
            Utilities.Log("Creating mat: " + mat.name + " at : " + materialPath);
        ...
        /// <returns></returns>
        public static Texture2D ImportTextureFromPath(string path, string localAssetDir,
            ImportEventRecord record,
            bool isNormal = false, bool isLinear = false)
        {
            ...
            if (ti == null)
            {
                Debug.LogWarning("Failed to get a texture importer for path: " + cleanPath +
                                 " verify texture has the correct settings manually");
            }
            else
            {
                if (isNormal)
                ...
                // if (DTUImporter.UseHighQualityTextures)
                // {

                if (ti.maxTextureSize != tex.width)
                {
                    ti.maxTextureSize = tex.width;
                    dirty = true;
                }
